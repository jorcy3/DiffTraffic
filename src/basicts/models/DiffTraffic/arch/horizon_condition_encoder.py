...
class HorizonConditionEncoder(nn.Module):
    """
    Encode horizon difficulty together with light temporal context.
    """
    ...
    def forward(
        ...
        if inputs_timestamps is None:
            ...
            last_tod = tod_index[:, -1:]
            last_dow = dow_index[:, -1:]
        ...
        return self.proj(features).to(base_prediction.dtype)
